...
curl -O
https://raw.githubusercontent.com/cemvarol/AZ-303-Labs/master/Lab-05/Lab-05-Resources.bash
...
to the next exercise. The deployment should take less than 5 minutes.
...
> **Note**: You can append **-blob** to distinguish from other
> connections.
...
> **Note**: This is expected, since the shared access signature does not
...
>
> Set-Content \'./az303Lab-05.html\' \'\<h3\> Hello from Windows Server
> this file is uploaded via AzCopy authenticated by SAS\</h3\>\'
...
> .\\azcopy cp \'./az303Lab-05.html\'
> \"https://\$storageAccountName.blob.core.windows.net/container01/az303Lab-05.html
> \<sas_token\>\"
...
> **Note**: If you want to allow anonymous access, you can set the
> public access level to **Blob (anonymous read access for blobs
> only)** or **Container (anonymous read access for containers and
> blobs)**.
...
3.  Select **+ File share** and create a file share with the following
    settings:
...
> **Note**: Azure Storage file share mapping uses the storage account
> name and one of two storage account keys as the equivalents of user
> name and password, respectively in order to gain access to the target
> share.
